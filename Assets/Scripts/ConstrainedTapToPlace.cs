...
namespace HoloToolkit.Unity.InputModule
{
    ...
    public class ConstrainedTapToPlace : MonoBehaviour, IInputClickHandler
    {
        public bool active = false;
        public CourseObjectCollection collect;
        public GameObject PopOverMenu;
        ...
        {
            if(!active) {
                PopOverController inst = Instantiate(PopOverMenu).GetComponent<PopOverController>();
                inst.Select(transform.gameObject);

                return;
            }
            // On each tap gesture, toggle whether the user is in placing mode.
            collect.SetSelectedObject(gameObject);
